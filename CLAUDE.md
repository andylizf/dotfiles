...
- **`setup.sh` exits 0 even when the nix build failed and nothing was installed.** The evaluation error is in its output, several screens up, and the previous generation stays live — so a deploy that "succeeded" can leave every change on the floor. Confirm the change reached its installed path before believing it landed.
- **`git add` a new file before deploying it.** Nix reads the flake through git, so an untracked file is invisible: the stanza referencing it aborts evaluation, and the whole deploy fails, including changes that have nothing to do with it. Editing a tracked file needs no staging.
