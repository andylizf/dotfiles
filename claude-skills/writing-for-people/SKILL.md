...
每一份写到他自己的稿子，扫两遍。**扫词**：名词前光秃的 `one`/`a`、`one of`、`part of`、`for N of them`、`helped with`、`assisted`、`some of`、`a portion of`、`just`、`only`，逐个处理：光秃的 `one`/`a` 换成定冠词，`just`/`only`/`a portion of` 这类纯缩小的直接删，`helped with`/`assisted` 是动词，删了会剩残句，换成他具体做的那件事。**扫人名**：稿子里每一个不是他的人名，按上面那条判一次；构造不是词，扫词那一遍抓不到它。两遍共用一个例外：改完会让读者得出错误结论就不改。
