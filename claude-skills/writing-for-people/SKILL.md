...
- **单音节压缩**：为省字把双音节词砍成单字，念出来像公文或古文，不像人说话。
  ...
  - 判据一，查词：**这个字有没有你日常真会说的双音节形式**：谢→谢谢、辩→辩解、
    ...
    “我**看**行”“这个我**认**”这种听着现成的搭配同样要换（→ 我觉得可以、这个我承认）。
    去、来、给这些本来就没有日常双音节形式的，查词这一步放过，交给判据二。
  - 判据二，查位置：**句号、问号、感叹号前的最后一个词不能是单字动词**。逗号前不算，
    ...
    被抬上去。命中的这句正好是全文最后一句时，先按「结尾」那一节定它落在请求还是承诺，
    判据二只查定下来之后的用词。
  - 定稿时把中文里的单字动词和单字压缩逐个找出来，对照这两条查一遍，报出命中几处、
    在哪几句，一处都没有也要说
...
全文交给 `writing-reviewer` subagent：评论、邮件、帖子、README、release note、文档、
给评审看的材料、公开仓库里的 PR 描述，以及任何 at 了某个人或整段写给某个 reviewer 的字。
它手上有穷举用的东西：英文换词表、英文套路清单、字频上限、标点、写他自己时的扫词和
扫人名。**这些不放在这份文件里，是因为它们只在成稿之后有用，而这份文件要在下笔之前
就起作用。**
...
字频上限、标点、扫词和扫人名，都在 `writing-reviewer.agent.md` 里，照着扫一遍再写。
