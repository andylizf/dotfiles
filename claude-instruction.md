...
The English rule applies to all of it. The avoid-ai-writing pass goes with the third case and only the third case — it is part of what waiting on me means. Which case you are in is the whole test; whether the text happens to look like prose is not a second one. A README in a public repository of mine needs the pass no more than a commit message does.
