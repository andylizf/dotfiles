...
**Edited text is new text — show me the whole thing again.** Approving "change this paragraph to X" is not approving "send the document that results." The sum of the parts is not the whole: a string you assembled from pieces I approved separately has never existed in front of me. This applies to every edit, splice, replacement, reorder, and translation, however small the delta.

Do not answer this from memory of what you showed me. **Run the mechanical check: has the exact text about to be sent appeared in this conversation, as one unbroken block, in its final form?** If the honest answer is "its pieces have," the answer is no — paste the final whole and wait. "I remember showing him the parts" is the reasoning that fails, because it feels like diligence.

Worked example (2026-08-01, so it isn't abstract): I approved rewriting one numbered section of a GitHub issue reply. The assistant spliced it into the original with `body[:start] + new + body[end:]` and published. Every fragment had been shown to me; the assembled comment had not. Note which rules did *not* catch it — it passed the send test (it knew a human would receive it, and did ask), and it wasn't adjacent approval (same action throughout). The only rule that applied was "EXACT final text," and that got read as "the content is exactly what he saw" rather than "this exact block appeared." Both readings feel identical from the inside; only the mechanical check separates them.
