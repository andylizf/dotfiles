...
in {
  ...
  config = {
    ...
    home.file.".zshenv".text = ''
      ...
      # The local activity notifier uses inherited PATH and no credentials.
      # Codex starts a shell for every notification, including before tool calls.
      case "''${ZSH_EXECUTION_STRING-}" in
        "$HOME/Projects/MyClaw/scripts/agent_awake_manager.py hook codex") return ;;
      esac
